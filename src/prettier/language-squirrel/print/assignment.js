...
function printAssignment(
  ...
  switch (layout) {
    ...
    case "fluid": {
      const hasObject = (path.node[rightPropertyName].type === "ObjectExpression") && !Array.isArray(rightDoc);
      ...
      return group([
        ...
        lineSuffixBoundary,
        indentIfBreak([(options.braceStyle === "allman") && hasObject ? softline : '', rightDoc], { groupId }),
      ]);
    }
...
function printAssignmentPattern(path, options, print) {
    return [print("left"), " = ", print("right")];
}

function printUpdateExpression(path, options, print) {
  const { node } = path;
  return [
...
export {
  ...
  printAssignmentPattern,
  printUpdateExpression,
