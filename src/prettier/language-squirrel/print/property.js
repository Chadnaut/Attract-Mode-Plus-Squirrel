...
function isStringKeySafeToUnquote(node, options) {
  ...
  if (
    ...
    !(
      ...
      (options.parser === "squirrel" && node.type === "PropertyDefinition")
    )
  ) {
    return true;
  }
  ...
  return (
    ...
    (key.type === "Identifier" ||
      (isNumericLiteral(key) &&
        ...
        !(options.parser === "squirrel" || options.parser === "babel-ts"))) &&
    (options.parser === "json" ||
      options.parser === "jsonc" ||
...
function printProperty(path, options, print) {
  ...
  return printAssignment(
    ...
    node.json ? ":" : ((path.parent.attributes && !options.attrSpacing) ? "=" : " ="),
    "value",
  );
}
