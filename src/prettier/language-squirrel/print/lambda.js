...
function printLambdaFunction(path, options, print, args = {}) {
  ...
  (function rec() {
    const { node } = path;
    const signatureDoc = printLambdaFunctionSignature(
      path,
      options,
      print,
      args,
  ...
  const signaturesDoc = printLambdaFunctionSignatures(path, args, {
    signatureDocs,
  ...
  bodyDoc = printLambdaFunctionBody(path, options, args, {
    bodyDoc,
    bodyComments,
    functionBody,
...
function printLambdaFunctionSignature(path, options, print, args) {
  const { node } = path;
  const parts = [];
 ...
 * @returns {Doc}
 */
function printLambdaFunctionSignatures(
  path,
  args,
 ...
 */
function printLambdaFunctionBody(
  path,
  options,
  args,
