...
import { getDocTargetBranch } from "./find";
...
import { getRequiredAttrs } from "../utils/program";
...
export const getDocProgramCompletions = (
    ...
    const template = getConfigValue<string>(constants.PACKAGE_TEMPLATE);
    if (!template) return [];

    const sourceName = program.sourceName;
    const dirname = path.basename(path.dirname(sourceName));
    const filename = path.basename(sourceName, constants.LANGUAGE_EXTENSION);
    const title =
        `${ucfirst(dirname)} ${ucfirst(filename)}`.replace(/^\W*/, "").trim() ||
        "Untitled";
    const d = new Date();

    // replace keywords
    let text = template.replace(/\${([^}:]*):?([^}]*)}/g, (match, name, pattern) => {
        switch (name) {
            case "title":
                return title;
            case "date":
    ...
    // add prefix asterisks
    text = "\n * " + text.replace(/\n/g, "\n * ") + "\n";

    // add requirements
    text = text.replace(
        /@requires\n/g,
        (match) => match + printDocAttrs(getRequiredAttrs(program)),
    );
...
export const getDocCompletions = (branch: AST.Node[]): CompletionItem[] => {
    ...
    switch (node?.type) {
        ...
        case "MethodDefinition":
            isConstructor = (<AST.MethodDefinition>node).kind === "constructor";
            if (isConstructor) {
